...
'''modify the quote search program discussed in lecture3 (scroll to the bottom) 
to allow user to enter multiple keywords and boolean operators'''
...
query = set(query.split()) #split query and convert to a set

#check which operator is in query
if "and" in query:
    query.remove('and')
    ...
    for i,quote in enumerate(data_list):
        for x in range(0, stop):
            ele = query[x]
            found_at.append(quote.find(ele))

        #go through found_at list and check if -1 exists, if so, dont print
        #if there's no -1 value in list, sort the list and go from lowest to highest index to print quote
        if (-1) not in found_at:
            found_at.sort()
            print("Found at ", i, "..."+quote[found_at[0]:found_at[-1]+50], "...")
        else:
            found_at = []
            continue
    ...
    #for loop for OR operator
    for i,quote in enumerate(data_list):
        for x in query:
	        found_at = quote.find(x)
	        if( found_at >= 0):
		        print("Found at ", i, "..."+quote[found_at:found_at+50], "...")

elif ("or" in query) and ("and" in query):
    query.remove('and')
    print("Performing AND search for: ", query)
    query = list(query)
    stop = len(query)
